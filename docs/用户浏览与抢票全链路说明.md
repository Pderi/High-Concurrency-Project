...
| 文档版本 | v1.5 |
| 编写日期 | 2026-07-23 |
| 代码基线 | P0～P2 + Redis 元数据预热（TTL 7 天 / SET·warmSession）+ Redis 库存预扣 + 建单 |
| 关联代码 | `controller/app/**`、`TktMetaCache`、`OrderGrabServiceImpl`、`TierStockRedisServiceImpl`、`OrderCreateServiceImpl`、`*OrderCreatePublisher` |
...
当前订单状态一般为 **待支付(10)**，后续 P3 模拟支付会迁到已支付并写电子票。
...
| **P3 模拟支付** | 待支付 → 已支付，生成电子票 | 未做 |
| **P4 超时关单** | 超时关单、余票回升 | `OrderCloseJob` 仅占位 |
| **P5 限流/压测** | 开售峰值体验 | 未做 |

```mermaid
flowchart LR
  Now[当前: 浏览+抢票+待支付] --> P3[P3 模拟支付+电子票]
  P3 --> P4[P4 超时关单+对账]
  P4 --> P5[P5 限流/观测/压测]
  Now -.-> Enh[增强: 限购 Redis 计数]
```
